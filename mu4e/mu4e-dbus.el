...
(defun mu4e-dbus-use-dbus ()
  (interactive)
  ;; Don't use `fset' here; use explicit forwarding.  If you `fset'
  ;; from the "proc" name to the "dbus" implementation, you can't
  ;; easily set up Edebug tracing versions of the "dbus"
  ;; implementation.
  (defun mu4e~proc-running-p ()
    "Forwarding to `mu4e~dbus-running-p'."
    (mu4e~dbus-running-p))
  (defun mu4e~call-mu (form)
    "Forwarding to `mu4e~dbus-call-mu'."
    (mu4e~dbus-call-mu form))
  (defun mu4e~proc-kill ()
    "Forwarding to `mu4e~dbus-kill'."
    (mu4e~dbus-kill)))
